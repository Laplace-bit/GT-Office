...
- Commit: recorded by git history
- Workspace: `/Users/dzlin/work/GT-Office`
- Recorded at: `2026-06-09 22:37 CST`
- Scope: focused terminal session-switching state hardening

### Changed

- `apps/desktop-web/src/shell/state/workspace-terminal-session-store.ts`
  - Cached workspace terminal documents now normalize retained `sessionSeq` values during hydration.
  - Missing, non-finite, and negative values reset to `0`; fractional sequence values are floored.
- `apps/desktop-web/tests/workspace-terminal-session-store.test.ts`
  - Added coverage for `NaN`, fractional, and negative cached session sequence values during workspace terminal document hydration.

### Requirement Mapping

- [APPLE_GRADE_CLI_AGENT_CONSOLE.md](APPLE_GRADE_CLI_AGENT_CONSOLE.md): Agent/session switching should preserve terminal state deterministically, including scroll/output continuity.
- `$native-feel-cross-platform-desktop`: T4 perceived performance and T6 cross boundaries intentionally; persisted WebView/session state should be normalized before reuse.
- `$ui-ux-pro-max`: revisiting a workspace should present predictable CLI Agent state instead of stale or invalid sequence-derived behavior.

### Passed

- `npm --workspace apps/desktop-web run test:unit -- workspace-terminal-session-store`
  - Result: passed.
  - Summary: `532` tests passed, `0` failed.
  - Note: the script still runs the full compiled web unit test set.
- `cargo check --workspace`
  - Result: passed.
- `npm run typecheck`
...
- Manual Tauri WebView validation of corrupted cached terminal sequence state across workspace switching.
- macOS and Windows packaged desktop runtime QA.

### Release Decision

This closes a narrow terminal session sequence hydration gap. It does not close the broader desktop runtime QA requirement.
