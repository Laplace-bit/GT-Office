import fs from 'node:fs/promises'
import fsSync from 'node:fs'
import net from 'node:net'
import os from 'node:os'
import path from 'node:path'
import { randomUUID } from 'node:crypto'

const DEFAULT_TIMEOUT_MS = 8000

export class BridgeClientError extends Error {
  code: string
  details?: unknown

  constructor(code: string, message: string, details: unknown = undefined) {
    super(message)
    this.name = 'BridgeClientError'
    this.code = code
    this.details = details
  }
}

export interface BridgeClient {
  request<T>(method: string, params?: unknown): Promise<T>
}

export interface BridgeRequestLike {
  request<T>(method: string, params?: unknown): Promise<T>
}

interface StateCandidate {
  key: string
  runtimePath: string
  directoryPath: string
}

export interface DirectoryStateSnapshot {
  directoryPath: string
  version?: string
  updatedAtMs?: number
  bridge?: {
    host?: string
    port?: number
    token?: string
    version?: string
  }
  workspaces?: Record<string, unknown>
}

function safeMtimeMs(filePath: string) {
  try {
    return fsSync.statSync(filePath).mtimeMs || 0
  } catch {
    return 0
  }
}

export function resolveStateCandidates(): StateCandidate[] {
  const overridePath = process.env.GTO_MCP_DIRECTORY_FILE
  const runtimeOverridePath = process.env.GTO_MCP_RUNTIME_FILE
  if ((overridePath && overridePath.trim()) || (runtimeOverridePath && runtimeOverridePath.trim())) {
    const directoryPath = overridePath?.trim()
      ? path.resolve(overridePath.trim())
      : path.join(path.dirname(path.resolve(runtimeOverridePath!.trim())), 'directory.json')
    const runtimePath = runtimeOverridePath?.trim()
      ? path.resolve(runtimeOverridePath.trim())
      : path.join(path.dirname(path.resolve(overridePath!.trim())), 'runtime.json')
    return [
      {
        key: `override:${path.dirname(runtimePath)}`,
        runtimePath,
        directoryPath,
      },
    ]
  }

  const candidates: StateCandidate[] = []
  const pushCandidate = (value?: string) => {
    if (!value) {
      return
    }
    const basePath = path.resolve(value)
    if (!candidates.some((candidate) => candidate.key === basePath)) {
      candidates.push({
        key: basePath,
        runtimePath: path.join(basePath, 'runtime.json'),
        directoryPath: path.join(basePath, 'directory.json'),
      })
    }
  }

  pushCandidate(path.join(os.homedir(), '.gtoffice', 'mcp'))

  if (process.env.WSL_DISTRO_NAME) {
    pushCandidate(path.join('/mnt/c/Users', os.userInfo().username, '.gtoffice', 'mcp'))
  }

  if (process.env.USERPROFILE && process.env.USERPROFILE.trim()) {
    pushCandidate(path.join(process.env.USERPROFILE.trim(), '.gtoffice', 'mcp'))
  }

  return candidates
}

export function compareStateCandidates(left: StateCandidate, right: StateCandidate) {
  return stateCandidateFreshness(right) - stateCandidateFreshness(left)
}

export function stateCandidateFreshness(candidate: StateCandidate) {
  return Math.max(safeMtimeMs(candidate.runtimePath), safeMtimeMs(candidate.directoryPath))
}

async function tryReadDirectoryWorkspaces(directoryPath: string) {
  try {
    const raw = await fs.readFile(directoryPath, 'utf8')
    const parsed = JSON.parse(raw) as { workspaces?: Record<string, unknown> }
    return parsed?.workspaces && typeof parsed.workspaces === 'object' ? parsed.workspaces : {}
  } catch {
    return {}
  }
}

async function resolveRuntimeCandidates(preferredWorkspaceId?: string) {
  const candidates = resolveStateCandidates()
  const scored = await Promise.all(
    candidates.map(async (candidate) => {
      const workspaces = await tryReadDirectoryWorkspaces(candidate.directoryPath)
      const matchesWorkspace = preferredWorkspaceId
        ? Object.prototype.hasOwnProperty.call(workspaces, preferredWorkspaceId)
        : false
      return {
        ...candidate,
        matchesWorkspace,
      }
    }),
  )

  scored.sort((left, right) => {
    if (left.matchesWorkspace !== right.matchesWorkspace) {
      return left.matchesWorkspace ? -1 : 1
    }
    return compareStateCandidates(left, right)
  })
  return scored
}

export async function loadDirectoryState(options: { workspaceId?: string } = {}): Promise<DirectoryStateSnapshot> {
  const preferredWorkspaceId =
    typeof options.workspaceId === 'string' && options.workspaceId.trim()
      ? options.workspaceId.trim()
      : ''
  const candidates = await resolveRuntimeCandidates(preferredWorkspaceId)
  const failures: Array<{ directoryPath: string; cause: string }> = []

  for (const candidate of candidates) {
    let raw: string
    try {
      raw = await fs.readFile(candidate.directoryPath, 'utf8')
    } catch (error) {
      failures.push({ directoryPath: candidate.directoryPath, cause: String((error as Error)?.message ?? error) })
      continue
    }

    try {
      const parsed = JSON.parse(raw) as DirectoryStateSnapshot
      return {
        ...parsed,
        directoryPath: candidate.directoryPath,
      }
    } catch (error) {
      failures.push({ directoryPath: candidate.directoryPath, cause: String((error as Error)?.message ?? error) })
    }
  }

  const directoryPath = candidates[0]?.directoryPath || path.join(os.homedir(), '.gtoffice', 'mcp', 'directory.json')
  throw new BridgeClientError('LOCAL_BRIDGE_UNAVAILABLE', `directory file not found: ${directoryPath}`, {
    directoryPath,
    candidates: candidates.map((candidate) => candidate.directoryPath),
    failures,
    preferredWorkspaceId,
  })
}

export async function loadRuntimeConfig(options: { workspaceId?: string } = {}) {
  const preferredWorkspaceId =
    typeof options.workspaceId === 'string' && options.workspaceId.trim()
      ? options.workspaceId.trim()
      : ''
  const candidates = await resolveRuntimeCandidates(preferredWorkspaceId)
  const failures: Array<{ runtimePath: string; cause: string }> = []

  for (const candidate of candidates) {
    let raw: string
    try {
      raw = await fs.readFile(candidate.runtimePath, 'utf8')
    } catch (error) {
      failures.push({ runtimePath: candidate.runtimePath, cause: String((error as Error)?.message ?? error) })
      continue
    }

    let parsed: { host?: string; port?: number; token?: string; version?: string }
    try {
      parsed = JSON.parse(raw)
    } catch (error) {
      failures.push({ runtimePath: candidate.runtimePath, cause: String((error as Error)?.message ?? error) })
      continue
    }

    const host = typeof parsed.host === 'string' && parsed.host ? parsed.host : '127.0.0.1'
    const port = Number(parsed.port)
    const token = typeof parsed.token === 'string' ? parsed.token : ''
    if (!Number.isInteger(port) || port <= 0 || !token) {
      failures.push({ runtimePath: candidate.runtimePath, cause: 'runtime file missing host/port/token' })
      continue
    }

    return {
      host,
      port,
      token,
      runtimePath: candidate.runtimePath,
      version: typeof parsed.version === 'string' ? parsed.version : 'unknown',
    }
  }

  const runtimePath = candidates[0]?.runtimePath || path.join(os.homedir(), '.gtoffice', 'mcp', 'runtime.json')
  throw new BridgeClientError('LOCAL_BRIDGE_UNAVAILABLE', `runtime file not found: ${runtimePath}`, {
    runtimePath,
    candidates: candidates.map((candidate) => candidate.runtimePath),
    failures,
    preferredWorkspaceId,
  })
}

export async function callBridge(
  method: string,
  params: Record<string, unknown> | undefined = {},
  options: { workspaceId?: string; timeoutMs?: number } = {},
) {
  const config = await loadRuntimeConfig({ workspaceId: options.workspaceId })
  const timeoutMs = Number.isFinite(options.timeoutMs) ? Number(options.timeoutMs) : DEFAULT_TIMEOUT_MS
  try {
    return await callBridgeWithConfig(config, method, params, timeoutMs)
  } catch (error) {
    if (!(error instanceof BridgeClientError)) {
      throw error
    }
    if (!['LOCAL_BRIDGE_UNAVAILABLE', 'LOCAL_BRIDGE_AUTH_FAILED'].includes(error.code)) {
      throw error
    }
    const directory = await loadDirectoryState({ workspaceId: options.workspaceId })
    const fallback = directory.bridge
    const port = Number(fallback?.port)
    const token = typeof fallback?.token === 'string' ? fallback.token : ''
    if (!fallback || !fallback.host || !Number.isInteger(port) || port <= 0 || !token) {
      throw error
    }
    return await callBridgeWithConfig(
      {
        host: fallback.host,
        port,
        token,
        runtimePath: directory.directoryPath,
        version: typeof fallback.version === 'string' ? fallback.version : 'unknown',
      },
      method,
      params,
      timeoutMs,
    )
  }
}

async function callBridgeWithConfig(
  config: { host: string; port: number; token: string; runtimePath: string; version: string },
  method: string,
  params: Record<string, unknown> | undefined,
  timeoutMs: number,
) {
  const requestId = randomUUID()

  return await new Promise((resolve, reject) => {
    const socket = net.createConnection({ host: config.host, port: config.port })
    let settled = false
    let responseBuffer = ''

    const fail = (error: unknown) => {
      if (settled) {
        return
      }
      settled = true
      socket.destroy()
      reject(error)
    }

    const finish = (value: unknown) => {
      if (settled) {
        return
      }
      settled = true
      socket.end()
      resolve(value)
    }

    const timer = setTimeout(() => {
      fail(
        new BridgeClientError('LOCAL_BRIDGE_TIMEOUT', `bridge request timed out after ${timeoutMs}ms`, {
          method,
          timeoutMs,
        }),
      )
    }, timeoutMs)

    socket.once('connect', () => {
      const payload = JSON.stringify({
        id: requestId,
        token: config.token,
        method,
        params,
      })
      socket.write(`${payload}\n`)
    })

    socket.on('data', (chunk) => {
      responseBuffer += chunk.toString('utf8')
      const newlineIndex = responseBuffer.indexOf('\n')
      if (newlineIndex < 0) {
        return
      }
      const line = responseBuffer.slice(0, newlineIndex).trim()
      responseBuffer = responseBuffer.slice(newlineIndex + 1)
      clearTimeout(timer)

      let envelope: {
        id?: string
        ok?: boolean
        data?: unknown
        error?: { code?: string; message?: string }
      }
      try {
        envelope = JSON.parse(line)
      } catch (error) {
        fail(
          new BridgeClientError('LOCAL_BRIDGE_UNAVAILABLE', 'bridge returned invalid JSON line', {
            line,
            cause: String((error as Error)?.message ?? error),
          }),
        )
        return
      }

      if (!envelope || envelope.id !== requestId) {
        fail(
          new BridgeClientError('LOCAL_BRIDGE_UNAVAILABLE', 'bridge response id mismatch', {
            expectedId: requestId,
            actualId: envelope?.id,
          }),
        )
        return
      }

      if (envelope.ok === false) {
        fail(
          new BridgeClientError(
            envelope.error?.code || 'LOCAL_BRIDGE_ERROR',
            envelope.error?.message || 'bridge request failed',
            envelope.error,
          ),
        )
        return
      }

      finish(envelope.data)
    })

    socket.on('error', (error) => {
      clearTimeout(timer)
      fail(
        new BridgeClientError('LOCAL_BRIDGE_UNAVAILABLE', 'bridge connection failed', {
          method,
          cause: String((error as Error)?.message ?? error),
        }),
      )
    })

    socket.on('end', () => {
      clearTimeout(timer)
      fail(
        new BridgeClientError('LOCAL_BRIDGE_UNAVAILABLE', 'bridge closed connection without response', {
          method,
        }),
      )
    })
  })
}

function readWorkspaceId(params?: unknown): string | undefined {
  if (!params || typeof params !== 'object') {
    return undefined
  }

  if ('workspaceId' in params && typeof params.workspaceId === 'string') {
    return params.workspaceId
  }

  if ('workspace_id' in params && typeof params.workspace_id === 'string') {
    return params.workspace_id
  }

  return undefined
}

export function createBridgeClient(bridge: BridgeRequestLike): BridgeClient {
  return {
    request<T>(method: string, params?: unknown) {
      return bridge.request<T>(method, params)
    },
  }
}

export function createDefaultBridgeClient(): BridgeClient {
  return {
    request<T>(method: string, params?: unknown) {
      return callBridge(method, params as Record<string, unknown> | undefined, {
        workspaceId: readWorkspaceId(params),
      }) as Promise<T>
    },
  }
}
