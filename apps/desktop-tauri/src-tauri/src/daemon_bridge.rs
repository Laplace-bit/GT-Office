...
fn emit_daemon_event(app: &AppHandle, event: Event) {
    if let Some((event_name, payload)) = daemon_event_payload(event) {
        let _ = app.emit(event_name, payload);
    }
}

fn daemon_event_payload(event: Event) -> Option<(&'static str, serde_json::Value)> {
    match event {
        Event::SearchChunk(chunk) => Some((
            "daemon/search_chunk",
            json!({
        ...
        Event::SearchBackpressure(backpressure) => Some((
        ...
        Event::SearchCancelled(cancelled) => Some((
            "daemon/search_cancelled",
            json!({
                "searchId": cancelled.search_id,
            }),
        )),
        Event::TerminalOutput(_) | Event::TerminalState(_) => None,
    }
}

#[cfg(test)]
#[path = "daemon_bridge_tests.rs"]
mod tests;
