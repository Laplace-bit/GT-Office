use super::{
    ...
    should_skip_log_prefix_line, should_skip_runtime_noise_line,
    should_skip_startup_banner_line, AppState, ExternalReplyDispatchPhase,
...
• 哈哈哈 😄\n\
  在的，你想先从哪个任务开始？",
    ...
    fn tui_status_bar_detection_is_tool_agnostic() {
        use super::{is_tui_status_bar_line, is_tui_padded_fragment};
...
› Write tests for @filename\n\
  人生不是短跑，而是一场漫长而值得认真走完的旅程。",
    ...
    fn should_skip_startup_banner_line_detects_connection_status() {
        ...
        assert!(should_skip_startup_banner_line("Authenticated successfully"));
    }
    ...
    fn should_skip_startup_banner_line_detects_config_messages() {
        ...
        assert!(!should_skip_startup_banner_line("Configuration error: invalid value"));
    }
    ...
    fn should_skip_log_prefix_line_detects_timestamps() {
        ...
        assert!(!should_skip_log_prefix_line("[IMPORTANT] This is not a timestamp"));
    }
    ...
    fn should_skip_runtime_noise_line_detects_status_words() {
        ...
        assert!(should_skip_runtime_noise_line("OK."));
        assert!(!should_skip_runtime_noise_line("Ready to process your request"));
        assert!(!should_skip_runtime_noise_line("OK"));
        assert!(!should_skip_runtime_noise_line("Done"));
    }
    ...
    fn should_skip_runtime_noise_line_detects_help_hints() {
        assert!(should_skip_runtime_noise_line("Type 'help' for more information"));
