...
import { resolveActiveRepositoryPath, restoreScopedRepositorySelection, shouldAdoptResolvedRepositorySelection, } from '../src/features/git/controllers/repository-selection-model.js';
function buildRepositorySummary(repositoryPath, overrides = {}) {
    return {
        repositoryPath,
...
test('prefers the remembered repository when it still exists in the workspace summary', () => {
    ...
    assert.equal(resolveActiveRepositoryPath('packages/beta', repositories, ''), 'packages/beta');
});
test('falls back to the primary repository when the remembered repository disappears', () => {
    ...
    assert.equal(resolveActiveRepositoryPath('packages/beta', repositories, 'packages/alpha'), 'packages/alpha');
});
test('restores repository selection per workspace and clears it when no workspace is active', () => {
    ...
    assert.equal(restoreScopedRepositorySelection('ws-a', selections), 'packages/alpha');
...
test('adopts the resolved repository only when controller state is out of sync', () => {
    ...
    assert.equal(shouldAdoptResolvedRepositorySelection({
