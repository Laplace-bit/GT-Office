...
import { getCompactPathTail, getCompactRepoLabel, getDirectoryLabel, getRepositoryDisplayLabel, hasStagedChanges, hasUnstagedChanges, resolveDiffScope, resolveDiscardKind, } from '../src/features/git/components/git-helpers.js';
test('repository display helpers keep multi-repo labels compact and stable', () => {
    ...
    assert.equal(getRepositoryDisplayLabel('packages/alpha', false, 'Workspace Root'), 'packages/alpha');
    assert.equal(getRepositoryDisplayLabel('packages/alpha', true, 'Workspace Root'), 'Workspace Root');
