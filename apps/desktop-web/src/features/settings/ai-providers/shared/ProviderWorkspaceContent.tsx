import type { ClaudeApiFormat, ClaudeAuthScheme, GeminiAuthMode } from '@shell/integration/desktop-api'
      ...
      {viewMode === 'list' ? (
        ...
        <section className="provider-workspace__panel">
          ...
          <div className="provider-workspace__form">
            ...
            {agentId === 'gemini' && seed.mode !== 'official' && <label className="provider-workspace__field"><span>{t(locale, '认证方式', 'Auth mode')}</span><select value={seed.authMode} onChange={(event) => { clearFeedback(); applySeed({ authMode: event.target.value as GeminiAuthMode, selectedType: event.target.value === 'oauth' ? 'oauth-personal' : 'gemini-api-key' }) }}><option value="oauth">OAuth</option><option value="api_key">API Key</option></select></label>}
            ...
            {agentId === 'gemini' && seed.mode !== 'official' && <label className="provider-workspace__field"><span>{t(locale, '运行类型', 'Runtime type')}</span><input type="text" value={seed.selectedType} readOnly className="is-readonly" /></label>}
            ...
            {(requiresApiKey || canReuseSecret) && <label className="provider-workspace__field"><span>{agentId === 'claude' ? seed.authScheme === 'anthropic_auth_token' ? 'ANTHROPIC_AUTH_TOKEN' : 'ANTHROPIC_API_KEY' : agentId === 'codex' ? 'OPENAI_API_KEY' : 'GOOGLE_API_KEY'}</span><input type="password" autoComplete="new-password" value={seed.apiKey} placeholder={canReuseSecret ? t(locale, '沿用已托管密钥', 'Reuse vaulted secret') : agentId === 'gemini' ? 'AIza...' : 'sk-...'} onChange={(event) => { clearFeedback(); applySeed({ apiKey: event.target.value }) }} /><small>{canReuseSecret ? t(locale, '留空则继续使用当前托管密钥。', 'Leave empty to keep using the vaulted secret.') : t(locale, '保存后会写入系统凭证库，不会明文落盘。', 'The credential is stored in the system keychain and never written in plaintext.')}</small></label>}
