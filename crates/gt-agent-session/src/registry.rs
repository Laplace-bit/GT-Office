...
impl SessionRegistry {
    pub fn open(db_path: PathBuf) -> SessionResult<Self> {
        ...
        let registry = Self { db_path, conn: Mutex::new(conn) };
        registry.ensure_schema()?;
        Ok(registry)
    }

    pub fn ensure_schema(&self) -> SessionResult<()> {
        let conn = self.conn.lock().map_err(|e| SessionError::Storage(e.to_string()))?;
    ...
    pub fn get(&self, gto_session_id: &str) -> SessionResult<Option<GtoSession>> {
        ...
        let row = stmt.query_row(params![gto_session_id], |row| Ok(row_to_session(row))).optional()?;
        Ok(row)
    }

    pub fn list_by_workspace(&self, workspace_id: &str, limit: u32, offset: u32) -> SessionResult<Vec<GtoSession>> {
        let conn = self.conn.lock().map_err(|e| SessionError::Storage(e.to_string()))?;
        let mut stmt = conn.prepare(
        ...
        let rows = stmt.query_map(params![workspace_id, limit, offset], |row| Ok(row_to_session(row)))?;
    ...
    pub fn list_cards_by_workspace(
        ...
        let map_row = |row: &rusqlite::Row| {
            Ok(SessionCard {
                ...
                agent_id: row.get(2)?,
                provider: Provider::from_str_opt(&row.get::<_, String>(3)?).unwrap_or(Provider::Claude),
                lifecycle: Lifecycle::from_str_opt(&row.get::<_, String>(4)?).unwrap_or(Lifecycle::Stopped),
        ...
        if let Some(provider) = provider {
            ...
            let rows = stmt.query_map(params![workspace_id, provider.as_str(), limit, offset], map_row)?;
    ...
    pub fn update_lifecycle(&self, gto_session_id: &str, lifecycle: Lifecycle, terminal_session_id: Option<&str>) -> SessionResult<()> {
        let conn = self.conn.lock().map_err(|e| SessionError::Storage(e.to_string()))?;
        let now = now_ms();
        let ended_at_ms: Option<u64> = if lifecycle == Lifecycle::Stopped { Some(now) } else { None };
        conn.execute(
    ...
    pub fn get_stats(&self, gto_session_id: &str) -> SessionResult<Option<SessionStats>> {
        ...
        let row = stmt.query_row(params![gto_session_id], |row| {
            Ok(SessionStats {
                gto_session_id: row.get(0)?,
    ...
    pub fn update_last_activity(&self, gto_session_id: &str, activity_ms: u64) -> SessionResult<()> {
        let conn = self.conn.lock().map_err(|e| SessionError::Storage(e.to_string()))?;
        conn.execute(
    ...
    pub fn merge_candidates(&self, candidates: &[ProviderSessionCandidate], workspace_id: &str) -> SessionResult<MergeResult> {
        ...
        Ok(MergeResult { new_count, updated_count })
    }
    ...
    pub fn backfill_missing_titles(
        ...
        let rows: Vec<(String, String, String)> = match provider {
            Some(provider_filter) => stmt
                .query_map(params![workspace_id, provider_filter.as_str(), limit], |row| {
                    Ok((
                        row.get::<_, String>(0)?,
                        row.get::<_, String>(1)?,
                        row.get::<_, String>(2)?,
...
fn row_to_session(row: &rusqlite::Row) -> GtoSession {
    GtoSession {
        ...
        provider: row.get::<_, String>(4).ok().and_then(|s| Provider::from_str_opt(&s)).unwrap_or(Provider::Claude),
        ...
        lifecycle: row.get::<_, String>(8).ok().and_then(|s| Lifecycle::from_str_opt(&s)).unwrap_or(Lifecycle::Stopped),
...
mod tests {
    ...
    fn make_session(id: &str, ws: &str, provider: Provider, lifecycle: Lifecycle) -> GtoSession {
        let now = now_ms();
        GtoSession {
            gto_session_id: id.to_string(), workspace_id: ws.to_string(),
            agent_id: "agent-1".to_string(), station_id: "station-1".to_string(),
            provider, provider_session_id: None,
            provider_log_path: Some(format!("/tmp/{}.jsonl", id)),
            terminal_session_id: if lifecycle == Lifecycle::Live { Some("term-1".to_string()) } else { None },
            lifecycle, title: Some("test session".to_string()), cwd: "/tmp".to_string(),
            started_at_ms: now, ended_at_ms: None, last_activity_at_ms: now,
            created_at_ms: now, updated_at_ms: now,
        }
    }

    #[test] fn test_schema_creates_tables() {
        let r = temp_registry();
        ...
        let tables: Vec<String> = conn.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").unwrap()
            .query_map([], |row: &rusqlite::Row| row.get(0)).unwrap()
            .filter_map(|r: Result<String,_>| r.ok()).collect();
        assert!(tables.contains(&"gto_sessions".to_string()));
        assert!(tables.contains(&"session_stats".to_string()));
    }

    #[test] fn test_schema_idempotent() { let r = temp_registry(); assert!(r.ensure_schema().is_ok()); assert!(r.ensure_schema().is_ok()); }

    #[test] fn test_insert_and_get() {
        let r = temp_registry();
        r.insert(&make_session("s1", "ws1", Provider::Claude, Lifecycle::Live)).unwrap();
    ...
    #[test] fn test_get_nonexistent() { assert!(temp_registry().get("nope").unwrap().is_none()); }

    #[test] fn test_list_by_workspace() {
        let r = temp_registry();
        r.insert(&make_session("s1", "ws1", Provider::Claude, Lifecycle::Live)).unwrap();
        r.insert(&make_session("s2", "ws1", Provider::Codex, Lifecycle::Stopped)).unwrap();
        r.insert(&make_session("s3", "ws2", Provider::Claude, Lifecycle::Live)).unwrap();
        assert_eq!(r.list_by_workspace("ws1", 100, 0).unwrap().len(), 2);
        assert_eq!(r.list_by_workspace("ws2", 100, 0).unwrap().len(), 1);
    }

    #[test] fn test_list_cards_join() {
        let r = temp_registry();
        r.insert(&make_session("s1", "ws1", Provider::Claude, Lifecycle::Stopped)).unwrap();
        r.update_stats(&SessionStats { gto_session_id: "s1".into(), git_start_commit: Some("abc".into()), git_end_commit: Some("def".into()), files_changed: 3, insertions: 42, deletions: 8, commits_ahead: 2, updated_at_ms: now_ms() }).unwrap();
    ...
    #[test] fn test_list_cards_without_stats() {
        let r = temp_registry();
        r.insert(&make_session("s1", "ws1", Provider::Claude, Lifecycle::Stopped)).unwrap();
        let cards = r.list_cards_by_workspace("ws1", None, 100, 0).unwrap();
    ...
    #[test] fn test_list_cards_by_provider() {
        let r = temp_registry();
        r.insert(&make_session("s1", "ws1", Provider::Claude, Lifecycle::Stopped)).unwrap();
        r.insert(&make_session("s2", "ws1", Provider::Codex, Lifecycle::Stopped)).unwrap();
        let claude_cards = r.list_cards_by_workspace("ws1", Some(Provider::Claude), 100, 0).unwrap();
        let codex_cards = r.list_cards_by_workspace("ws1", Some(Provider::Codex), 100, 0).unwrap();
    ...
    #[test] fn test_update_lifecycle_resume() {
        let r = temp_registry();
        r.insert(&make_session("s1", "ws1", Provider::Claude, Lifecycle::Stopped)).unwrap();
        r.update_lifecycle("s1", Lifecycle::Live, Some("term-new")).unwrap();
        assert_eq!(r.get("s1").unwrap().unwrap().terminal_session_id.as_deref(), Some("term-new"));
    }

    #[test] fn test_mark_all_live_stopped() {
        let r = temp_registry();
        r.insert(&make_session("s1", "ws1", Provider::Claude, Lifecycle::Live)).unwrap();
        r.insert(&make_session("s2", "ws1", Provider::Codex, Lifecycle::Live)).unwrap();
        r.insert(&make_session("s3", "ws1", Provider::Claude, Lifecycle::Stopped)).unwrap();
        assert_eq!(r.mark_all_live_stopped().unwrap(), 2);
        assert_eq!(r.get("s1").unwrap().unwrap().lifecycle, Lifecycle::Stopped);
    }

    #[test] fn test_update_and_get_stats() {
        let r = temp_registry();
        r.insert(&make_session("s1", "ws1", Provider::Claude, Lifecycle::Stopped)).unwrap();
        r.update_stats(&SessionStats { gto_session_id: "s1".into(), git_start_commit: Some("abc".into()), git_end_commit: Some("def".into()), files_changed: 5, insertions: 100, deletions: 20, commits_ahead: 3, updated_at_ms: now_ms() }).unwrap();
        let stats = r.get_stats("s1").unwrap().unwrap();
        assert_eq!(stats.files_changed, 5);
        assert_eq!(stats.commits_ahead, 3);
    }

    #[test] fn test_merge_candidates_new() {
        let r = temp_registry();
        let result = r.merge_candidates(&[ProviderSessionCandidate {
            provider: Provider::Claude, provider_session_id: None,
            log_path: PathBuf::from("/tmp/claude-1.jsonl"), cwd: PathBuf::from("/tmp"),
            modified_at_ms: now_ms(), first_user_message: None,
        }], "ws1").unwrap();
        assert_eq!(result.new_count, 1);
        assert_eq!(result.updated_count, 0);
    }

    #[test] fn test_merge_candidates_dedup() {
        let r = temp_registry();
        let c = ProviderSessionCandidate { provider: Provider::Claude, provider_session_id: None, log_path: PathBuf::from("/tmp/claude-1.jsonl"), cwd: PathBuf::from("/tmp"), modified_at_ms: now_ms(), first_user_message: None };
    ...
    #[test] fn test_update_title() {
        let r = temp_registry();
        r.insert(&make_session("s1", "ws1", Provider::Claude, Lifecycle::Live)).unwrap();
        r.update_title("s1", "new title").unwrap();
        assert_eq!(r.get("s1").unwrap().unwrap().title.as_deref(), Some("new title"));
    }
}